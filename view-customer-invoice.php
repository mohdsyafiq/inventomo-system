...
// --- GET THE INVOICE ID ---
$ci_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;
...
if ($ci_id === 0) {
    if ($is_modal) {
        echo "<p class='text-danger'>Error: No Invoice ID provided.</p>";
        exit;
    } else {
...
if ($result_po->num_rows > 0) {
    ...
    // Invoices Items Query - MODIFIED WITH LEFT JOIN AND COALESCE
    $sql_items = "
        SELECT
            ci.id, ci.invoice_number, ci.invoice_date, ci.total_amount, ci.status,
            cs.companyName AS customer_name,
            iis.quantity, iis.sale_price, iis.line_total,
        ...
        JOIN
            customer_supplier cs ON ci.customer_id = cs.id
        LEFT JOIN
            invoice_items iis ON ci.id = iis.invoice_id -- <<<--- THIS IS THE CRITICAL CHANGE
        LEFT JOIN
            inventory_item ii ON iis.item_id = ii.itemID
        WHERE
            ci.id = ?
    ";
    ...
    while ($row = $result_items->fetch_assoc()) {
        ...
        $total_amount += (($row['quantity'] ?? 0) * ($row['sale_price'] ?? 0));
    }
    $stmt_items->close();

} else {
    if ($is_modal) {
        echo "<p class='text-danger'>Error: Invoices not found.</p>";
        exit;
    } else {
        die("Invoices not found.");
    }
}
$stmt_po->close();
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="text-align: right;"><?php echo format_rm($item['sale_price'] ?? 0); ?></td>
                        <td style="text-align: right;"><?php echo format_rm(($item['quantity'] ?? 0) * ($item['sale_price'] ?? 0)); ?></td>
                    </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
...
<head>
    ...
    <title>View Invoices - Inventomo</title>

    <meta name="description" content="View Invoices Details" />
            ...
            <div class="layout-page">
                ...
                <div class="content-wrapper">
                    <div class="container-xxl flex-grow-1 container-p-y">
                        <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Orders & Billing /</span> View Invoice</h4>
                        ...
                        <div class="card invoice-card" id="invoice-content">
                            <div class="invoice-header d-flex justify-content-between align-items-start">
                                ...
                                <div class="text-end">
                                    <h1 class="invoice-title">INVOICES</h1>
                                    <div class="invoice-details text-muted">
                                        <p class="mb-1"><strong>Invoice Number:</strong> <?php echo htmlspecialchars($po_data['invoice_number'] ?? 'N/A'); ?></p>
                                        <p class="mb-1"><strong>Order Date:</strong> <?php echo date('F j, Y', strtotime($po_data['invoice_date'] ?? 'now')); ?></p>
                                        <p class="mb-1">
                                            ...
                                            <span class="badge
                                                ...
                                                switch ($current_status) {
                                                    case 'unpaid': echo 'bg-label-warning'; break;
                                                    case 'paid': echo 'bg-label-success'; break;
                                            ...
                                            <?php if ($current_status !== 'paid'): // Only show button if not already approved ?>
                                                <a href="update-invoices-status.php?id=<?php echo htmlspecialchars($ci_id); ?>&status=paid"
